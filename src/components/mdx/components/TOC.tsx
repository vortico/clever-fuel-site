...
function getItemFromAnchor(anchor: HTMLAnchorElement, children?: TOCItem[]): TOCItem {
  return {
    active: false,
    anchor: anchor,
    section: document.querySelector<HTMLAnchorElement>(`${anchor.hash}`)!.parentElement!,
    children,
  }
}
...
interface TOCProps extends ComponentProps<'nav'> {
  title: string
  titleSlug?: string
...
export default function TOC({ title, titleSlug, activeClassNames, children, ...props }: TOCProps) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        toc.forEach((firstLevel) => {
          if (firstLevel === firstLevelActive) {
          ...
          firstLevel.children?.forEach((secondLevel) => {
            if (secondLevel === secondLevelActive) {
  ...
  return (
    <nav
      {...props}
      className="prose-toc fixed top-20 right-[max(0px,calc(50%-45rem))] bottom-0 z-20 hidden w-72 overflow-y-auto px-8 lg:block"
    >
      <h5 className="text-primary-900 dark:text-primary-100 mt-8 mb-4 text-sm font-semibold">On this page</h5>
      {titleSlug && (
