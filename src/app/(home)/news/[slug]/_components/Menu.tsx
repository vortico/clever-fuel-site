...
function BackButton() {
  return (
    <Link
      href="/news/"
      className="group text-primary-600 hover:text-brand-500 flex items-center space-x-2 transition-colors duration-200"
    >
      <IconChevronsLeft className="h-4" />
      <span className="font-semibold">Go back to news</span>
    </Link>
  )
}

function Metadata({ document }: { document: NewsDocument }) {
  ...
  return (
    <header className="flex w-full flex-col justify-center gap-12 md:flex-row xl:flex-col">
      <div>
        <h5 className="text-primary-700 mb-4">{`${
      ...
      <div>
        <h5 className="text-primary-700 mb-4">Publication</h5>
        <div className="pl-4 text-sm">
          <time dateTime={document.metadata.date.toString()}>{dateFormatter.format(document.metadata.date)}</time>
        </div>
      </div>
      <div>
        <h5 className="text-primary-700 mb-4">Reading Time</h5>
        <div className="pl-4 text-sm">
...
export default function Menu({ document }: MenuProps) {
  return (
    <div className="mb-4 flex flex-col md:mb-8 xl:mb-0">
      <div className="border-primary-200 mb-5 hidden border-b pb-5 xl:block">
        <BackButton />
      </div>
